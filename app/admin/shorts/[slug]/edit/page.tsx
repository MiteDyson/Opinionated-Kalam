...
export default function EditShortPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!res.ok) throw new Error("Short read not found");
        const data = await res.json();
        if (data.type && data.type !== "short") {
          router.replace(`/admin/${data.type === "podcast" ? "podcasts" : "articles"}/${slug}/edit`);
          return;
        }
        setTitle(data.title ?? "");
  ...
  const handleSave = async (publishNow?: boolean) => {
    ...
    const newStatus = publishNow !== undefined ? (publishNow ? "published" : "draft") : status;
    ...
    <div style={{ minHeight: "100vh", backgroundColor: BG, color: TEXT }}>
      ...
      <div style={{ backgroundColor: TEXT, padding: "0 14px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 52, position: "sticky", top: 0, zIndex: 10, gap: 8 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0, overflow: "hidden" }}>
          <button onClick={() => router.push("/admin")} style={{ background: "none", border: "none", color: "#888", cursor: "pointer", display: "flex", alignItems: "center", gap: 4, fontSize: "0.83rem", fontFamily: "'Inter', sans-serif", flexShrink: 0 }}>
          ...
          <span style={{ fontFamily: "'DM Serif Display', serif", fontSize: "1rem", color: "white", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", display: "flex", alignItems: "center", gap: 6 }}>
            <Zap size={14} style={{ color: "white" }} /> Edit Short Read
          </span>
          {!loading && (
            <span style={{ fontSize: "0.72rem", padding: "2px 8px", borderRadius: 4, fontFamily: "'Inter', sans-serif", fontWeight: 600, backgroundColor: status === "published" ? "rgba(76,140,80,0.2)" : "rgba(255,200,0,0.2)", color: status === "published" ? "#3a7a3e" : "#8a6a00", marginLeft: 4 }}>
              {status}
            </span>
          )}
        </div>
        <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
          {error && <span style={{ color: "#ff6b6b", fontSize: "0.72rem", fontFamily: "'Inter', sans-serif", maxWidth: 120, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{error}</span>}
          <button onClick={() => router.push("/admin")} disabled={saving || loading} style={{ padding: "0 10px", height: 30, borderRadius: 6, border: "1px solid rgba(255,255,255,0.18)", backgroundColor: "transparent", color: "#ccc", cursor: "pointer", fontSize: "0.75rem", fontWeight: 600, fontFamily: "'Inter', sans-serif", whiteSpace: "nowrap", flexShrink: 0 }}>Cancel</button>
          <button onClick={() => handleSave()} disabled={saving || loading} style={{ padding: "0 12px", height: 30, borderRadius: 6, border: "none", backgroundColor: TERRA, color: TEXT, cursor: "pointer", fontSize: "0.75rem", fontWeight: 700, fontFamily: "'Inter', sans-serif", opacity: saving ? 0.7 : 1, whiteSpace: "nowrap", flexShrink: 0 }}>
